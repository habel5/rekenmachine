...
class Calculator:
    def __init__(self):
        ...
        self.window.iconbitmap(folder + "\\calc_icon.ico")
    ...
    def bind_keys(self):
        self.window.bind("<Return>", lambda event: self.evalutate())
        for key in self.digits:
            self.window.bind(str(key), lambda event, digit=key: self.add.expression(digit))
    ...
    def create_equals_button(self):
        button = tk.Button(self.buttons_frame, text="=", bg=LIGHT_BLUE, fg=LABEL_COLOR, font=DEFAULT_FONT_STYLE, borderwidth=0, command= self.evalutate)
